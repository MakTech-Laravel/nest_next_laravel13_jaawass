...
<head>
    ...
    <title>{{ __('mail.manufacturer_admin_message.subject') }}</title>
</head>
...
<body style="margin:0;padding:0;background-color:#f4f4f5;font-family:system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
    <span style="display:none !important;visibility:hidden;font-size:1px;color:#f4f4f5;line-height:1px;max-height:0;max-width:0;opacity:0;overflow:hidden;">
        {{ __('mail.manufacturer_admin_message.preheader') }}
    </span>
                    ...
                    <tr>
                        <td style="padding:28px 28px 12px 28px;background-color:#111827;">
                            ...
                            <h1 style="margin:0;font-size:24px;line-height:1.3;font-weight:600;color:#f8fafc;">
                                {{ __('mail.manufacturer_admin_message.subject') }}
                            </h1>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding:28px;">
                            <p style="margin:0 0 16px 0;font-size:15px;line-height:1.6;color:#111827;">
                                {{ __('mail.manufacturer_admin_message.greeting', ['name' => $manufacturerName]) }}
                            </p>
                            <p style="margin:0 0 24px 0;font-size:15px;line-height:1.6;color:#374151;">
                                {{ __('mail.manufacturer_admin_message.intro', ['admin' => $adminName, 'company' => $companyName]) }}
                            </p>
                            ...
                            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="background-color:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;margin:0 0 24px 0;">
                                <tr>
                                    <td style="padding:18px 20px;">
                                        <p style="margin:0 0 8px 0;font-size:12px;font-weight:700;letter-spacing:0.08em;color:#6b7280;text-transform:uppercase;">
                                            {{ __('mail.manufacturer_admin_message.message_heading') }}
                                        </p>
                                        <p style="margin:0;font-size:15px;line-height:1.7;color:#111827;white-space:pre-wrap;">{{ $adminMessage }}</p>
                                    </td>
                                </tr>
                            </table>

                            <p style="margin:0 0 8px 0;font-size:13px;font-weight:700;color:#111827;">
                                {{ __('mail.manufacturer_admin_message.ticket_heading') }}
                            </p>
                            <p style="margin:0 0 24px 0;font-size:14px;line-height:1.6;color:#374151;">
                                {{ __('mail.manufacturer_admin_message.ticket_body', ['subject' => $ticketSubject]) }}
                            </p>

                            <table role="presentation" cellspacing="0" cellpadding="0" border="0">
                                <tr>
                                    <td style="border-radius:8px;background-color:#1d4ed8;">
                                        <a href="{{ $ticketUrl }}" style="display:inline-block;padding:14px 24px;font-size:15px;font-weight:600;color:#ffffff;text-decoration:none;">
                                            {{ __('mail.manufacturer_admin_message.cta') }}
                                        </a>
                                    </td>
                                </tr>
                            </table>
                    ...
                    <tr>
                        <td style="padding:20px 28px;background-color:#f9fafb;border-top:1px solid #e5e7eb;">
                            <p style="margin:0;font-size:12px;line-height:1.6;color:#6b7280;text-align:center;">
                                {{ __('mail.manufacturer_admin_message.footer') }}
                            </p>
                        </td>
                    </tr>
                </table>
