...
<body style="margin:0;padding:0;background-color:#f4f4f5;font-family:system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;">
    <span style="display:none !important;visibility:hidden;font-size:1px;color:#f4f4f5;line-height:1px;max-height:0;max-width:0;opacity:0;overflow:hidden;">
        {{ __('mail.subscription_created.preheader') }}
    </span>
                    ...
                    <tr>
                        <td style="padding:28px 28px 12px 28px;background-color:#111827;">
                            ...
                            <h1 style="margin:0;font-size:24px;line-height:1.3;font-weight:600;color:#f8fafc;">
                                {{ __('mail.subscription_created.subject') }}
                            </h1>
                        </td>
                    </tr>
                    <tr>
                        <td style="padding:28px;">
                            <p style="margin:0 0 16px 0;font-size:15px;line-height:1.6;color:#111827;">
                                {{ __('mail.subscription_created.greeting', ['name' => $manufacturerName]) }}
                            </p>
                            <p style="margin:0 0 24px 0;font-size:15px;line-height:1.6;color:#374151;">
                                {{ __('mail.subscription_created.intro', ['plan' => $planName]) }}
                            </p>
                                ...
                                <tr>
                                    <td style="padding:18px 20px;">
                                        <p style="margin:0 0 12px 0;font-size:12px;font-weight:700;letter-spacing:0.08em;color:#6b7280;text-transform:uppercase;">
                                            {{ __('mail.subscription_created.details_heading') }}
                                        </p>
                                        <p style="margin:0 0 8px 0;font-size:14px;color:#374151;"><strong>{{ __('mail.subscription_created.billing_interval') }}:</strong> {{ $billingInterval }}</p>
                                        <p style="margin:0 0 8px 0;font-size:14px;color:#374151;"><strong>{{ __('mail.subscription_created.starts_at') }}:</strong> {{ $startsAt }}</p>
                                        <p style="margin:0 0 8px 0;font-size:14px;color:#374151;"><strong>{{ __('mail.subscription_created.ends_at') }}:</strong> {{ $endsAt }}</p>
                                        @if ($paidAmount !== null)
                                            <p style="margin:0;font-size:14px;color:#374151;"><strong>{{ __('mail.subscription_created.paid_amount') }}:</strong> {{ $paidAmount }}</p>
                                        @endif
                                    </td>
                                </tr>
                            </table>
                                ...
                                <tr>
                                    <td style="border-radius:8px;background-color:#1d4ed8;">
                                        <a href="{{ $plansUrl }}" style="display:inline-block;padding:14px 24px;font-size:15px;font-weight:600;color:#ffffff;text-decoration:none;">
                                            {{ __('mail.subscription_created.cta') }}
                                        </a>
                                    </td>
                                </tr>
                            </table>
                    ...
                    <tr>
                        <td style="padding:20px 28px;background-color:#f9fafb;border-top:1px solid #e5e7eb;">
                            <p style="margin:0;font-size:12px;line-height:1.6;color:#6b7280;text-align:center;">
                                {{ __('mail.subscription_created.footer') }}
                            </p>
                        </td>
                    </tr>
                </table>
