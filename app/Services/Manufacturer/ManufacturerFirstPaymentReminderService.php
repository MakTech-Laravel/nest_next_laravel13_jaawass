...
class ManufacturerFirstPaymentReminderService
{
    ...
    public function sendReminder(User $user): void
    {
        ...
        if ($user->manufacturer_first_payment_reminder_sent_at !== null) {
            return;
        }

        MailNotificationHelper::sendIfEmail($user, function (string $email) use ($user): void {
            $this->mailingService->send($email, MailTemplate::ManufacturerFirstPaymentReminder, [
        ...
        $user->forceFill(['manufacturer_first_payment_reminder_sent_at' => now()])->save();
    }
}
