...
test('faq update response uses requested locale translation', function () {
    ...
    $this->withHeader('Authorization', 'Bearer '.$token)
        ->putJson("/api/v1/admin/faqs/{$faq->id}?locale=ar", $payload1)
        ->assertOk()
        ->assertJsonPath('data.question', 'New Q 1');
    ...
    $this->withHeader('Authorization', 'Bearer '.$token)
        ->putJson("/api/v1/admin/faqs/{$faq->id}?locale=ar", $payload2)
        ->assertOk()
        ->assertJsonPath('data.question', 'New Q 2');
});
